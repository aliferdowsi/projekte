...
void GAMEPLAY::setTurn(){
    ...
    if(turn-2 < 0){
        players.at(players.size()-2+last_round_bigblind).is_SMALL_BLIND = true;
        players.at(players.size()-2+last_round_bigblind).round_paid_amount = SMALL_BLIND_AMOUNT;
        players.at(players.size()-2+last_round_bigblind).chips -= SMALL_BLIND_AMOUNT;
    } else {
    ...
    if(turn-1 < 0){
        players.at(players.size()-1).is_BIG_BLIND = true;
        players.at(players.size()-1).small_blind_paid = true;
        players.at(players.size()-1).round_paid_amount = BIG_BLIND_AMOUNT;
        players.at(players.size()-1).chips -= BIG_BLIND_AMOUNT;
    } else {
